...
function togglePaymentDetails(e) {
  ...
  const paypalUsernameContainer = document.getElementById("paypalUsernameContainer");

  // Hide payment containers by adding the '.hide' class to each of them
  creditCardNumberContainer.classList.add("hide");
  paypalUsernameContainer.classList.add("hide");
  ...
  const paymentFields = [creditCardContainer, paypalUsernameContainer];
  ...
  const selectedPaymentMethod = theForm.paymentMethod.value;
  if (selectedPaymentMethod === "creditCard") {
    creditCardContainer.classList.remove("hide");
    creditCardContainer.querySelector("input").required = true;
  } else if (selectedPaymentMethod === "paypal") {
    paypalUsernameContainer.classList.remove("hide");
    paypalUsernameContainer.querySelector("input").required = true;
  }
}
